...
| `0002` | `./` | `prep_ubuntu_system: skip apt.llvm.org and apt.kitware.com on Ubuntu 26.04 (resolute)` | Ubuntu 26.04 already ships `clang-17`, `clang-20`, `llvm-20-dev` (1:20.1.8) and `cmake` (4.2.3) in main/universe, so the third-party `apt.llvm.org` and `apt.kitware.com` repositories that `prep_ubuntu_system` adds for older Ubuntu releases are redundant. Worse, `apt.llvm.org` has not yet published a `resolute` pocket, so adding it forces `apt-get update` to fail with a 404. Wrap both repository setups in a `case "$OS_CODENAME"` guard that opts out on `resolute` and falls back to the distro toolchain. The Dockerfile pre-installs `clang-20` so `install_llvm`'s `command -v clang-20` short-circuits its `apt.llvm.org/llvm.sh 20` call as well. Distro-Scope: `ubuntu-26.04`. |
